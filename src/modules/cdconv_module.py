...
class CDConv(MessagePassing):
    ...
    def reset_parameters(self):
        ...
        kaiming_uniform(self.W.data, size=[len(self.kernel_channels) * self.in_channels, self.out_channels])
...
# edge_index [2,] x[16,500,8] pos[32000,3] 
        seq = seq.view(-1, 1)
        ori = ori.view(-1, 9)
        x=x.reshape(-1,x.shape[2])
        out = self.propagate(edge_index, x=(x, None), pos=(pos, pos), seq=(seq, seq), ori=(ori, ori), size=None)
